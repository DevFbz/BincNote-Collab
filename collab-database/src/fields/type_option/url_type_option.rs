...
mod tests {
  ...
  fn url_cell_to_serde() {
    ...
    let cell_writer: Box<dyn TypeOptionCellReader> = Box::new(url_type_option);
    {
      ...
      cell.insert(CELL_DATA.into(), "https://bincnote.io".into());
      let serde_val = cell_writer.json_cell(&cell);
      assert_eq!(serde_val, "https://bincnote.io");
    }
  }
  ...
  fn url_serde_to_cell() {
    ...
    {
      let cell: Cell =
        cell_writer.convert_json_to_cell(Value::String("https://bincnote.io".to_string()));
      let data: String = cell.get_as::<String>(CELL_DATA).unwrap();
      assert_eq!(data, "https://bincnote.io");
    }
  }
}
